...
export default function VendorForgotPasswordForm() {
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      const result = await vendorAuthService.forgotPassword(normalizedEmail);
      ...
      toast.success(
        result.message || 'If this email is registered, a password reset link has been sent.'
      );
    } catch (error) {
      toast.error(
    ...
    <main className="relative flex min-h-screen items-center justify-center overflow-hidden bg-gradient-to-br from-[#F4F8FA] via-[#FAFAFA] to-[#FFFFFF] px-4 py-10">
      ...
      <section className="relative z-10 w-full max-w-md rounded-3xl border border-[#EAEAEA] bg-[#FFFFFF]/90 p-6 shadow-[0_20px_60px_-32px_rgba(13,71,161,0.45)] backdrop-blur-xl sm:p-8">
        ...
        <p className="mt-3 text-sm leading-6 text-[#666666]">
          Enter your vendor account email and we will send you a secure password reset link.
        </p>
        ...
        <p className="mt-8 text-center text-sm text-[#666666]">
          ...
          <Link href={ROUTES.VENDOR.LOGIN} className="font-semibold text-[#4285F4] hover:text-[#0D47A1] hover:underline">
            Back to login
          </Link>
        </p>
      </section>
