...
import { useVendorForgotPassword } from '@/hooks/vendor/useVendorForgotPassword';

export default function VendorForgotPasswordForm() {
	const { email, loading, success, error, setEmail, handleSubmit, goToLogin } = useVendorForgotPassword();

	return (
		<div className="min-h-screen flex items-center justify-center bg-gray-50 p-4">
			<div className="bg-white shadow-2xl rounded-2xl p-8 w-full max-w-md">
				<h1 className="text-3xl font-semibold text-center text-[#5a6c7d] mb-3">Forgot Password</h1>
				...
				{!success && (
					<div className="mt-6 text-center">
						<Link href={ROUTES.VENDOR.LOGIN} className="text-[#1e88e5] hover:underline text-sm font-medium">
							← Back to Login
						</Link>
					</div>
				)}
