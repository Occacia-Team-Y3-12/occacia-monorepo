...
_MAX_HISTORY   = 10   # max messages passed to Groq (~5 turns)
...
def _build_system_prompt(
    ...
    if event_type_upper == "GROUP":
        ...
        persona_section = (
            ...
            "    • Then move to Phase 1 (planning facts).\n"
            "    • Do not ask about budget, food, music, or colour preferences in Phase 0.\n"
            "      Only collect those if the user volunteers them naturally.\n"
            "    • If the user gives name AND other details in one message, extract all and move on.\n\n"
        )
    else:
        persona_section = (
            "PLANNING MODE — persona already saved:\n"
            "  Do NOT ask any persona questions. Move directly to planning facts or the next phase.\n\n"
        )
...
R6. Banned opener patterns — never start a reply with any of these styles:
    • Generic greeting that ignores the user's message (e.g. a rote hello + offer to help)
    • Hollow enthusiasm that adds nothing (e.g. "Great!" / "Certainly!" as a standalone opener)
    • AI self-reference (e.g. identifying yourself as an AI/language model)
    • Any opener that does not directly react to what the user just said.
...
def _get_msg_attr(msg: Any, attr: str, default: str = "") -> str:
    """Read an attribute from either an ORM object or a plain dict."""
    if isinstance(msg, dict):
        return str(msg.get(attr, default) or default)
    return str(getattr(msg, attr, default) or default)


def _build_messages(content: str, history: list, system_prompt: str) -> list[dict]:
    """
    Build the Groq messages array.
    Accepts history as ORM EventChatMessage objects OR plain dicts
    (tests pass dicts; production passes ORM objects).
    sender values: "CUSTOMER" | "AI" | "user" | "assistant"
    History arrives oldest-first; we cap at _MAX_HISTORY before sending.
    """
...
def _compute_missing(ctx: dict, ef: dict) -> list[str]:
    ...
    Handles both snake_case keys (from DB event_context) and camelCase keys
    (from AI eventFacts or test fixtures) so tests and production both work.
    """
    missing: list[str] = []

    guest_known = (
        ctx.get("guest_count") or ctx.get("guestCount")
        or ef.get("guestCount") or ef.get("guest_count")
    )
    if not guest_known:
        missing.append("guestCount")

    budget_known = (
        ctx.get("budget_total") or ctx.get("budgetTotal")
        or ctx.get("budget_per_head") or ctx.get("budgetPerHead")
        or ef.get("budgetTotal") or ef.get("budget_total")
        or ef.get("budgetPerHead") or ef.get("budget_per_head")
    )
    if not budget_known:
        missing.append("budget")

    date_known = (
        ctx.get("event_date") or ctx.get("date")
        or ef.get("date") or ef.get("event_date")
    )
    if not date_known:
        missing.append("date")

    expectations_known = (
        ctx.get("expectations") or ef.get("expectations")
    )
...
class GroqAIService:
    ...
    async def recommend_offerings_for_task(
        ...
        *,
        ...
        system_msg = (
            "You are a vendor offering ranking engine for Occacia, Sri Lanka. "  # nosec B608
